...
- ✅ **灵活扩展** — 兼容任何 OpenAI 风格 API 服务商，支持按服务商配置自定义请求头

---

## DEMO
<p align="center">
  <img src="assets/demo-01.png" width="41%">
  <img src="assets/demo-02.png" width="16%">
  <img src="assets/demo-03.png" width="41%">
</p>
...
### 🔒 安全设计
- **敏感配置加密**：API Key 与服务商自定义请求头使用 AES-256-GCM 存储，密钥由 `APP_SECRET` 经 SHA-256 派生
...
```bash
git clone https://github.com/DsureD/light-chat.git
cd light-chat
npm install
```
...
1. 登录后进入 `/admin`
2. 添加服务商：填写名称、API Base URL、API Key；需要模拟特定客户端时，可展开「高级配置」添加自定义请求头
3. 点击「查询导入」自动拉取该服务商的模型列表；不支持列表接口的可手动添加

#### 服务商自定义请求头

- 请求头按服务商独立配置，会统一应用于模型列表查询、聊天补全和图片生成请求。
- 支持 `User-Agent`、`Referer`、`Origin` 以及各类 `X-*` 请求头，适用于要求特定客户端标识的上游服务。
- 请求头由 Light Chat 服务端发送，不受浏览器跨域和浏览器禁止修改 `User-Agent` 等限制。
- `Authorization`、`Content-Type`、`Host`、`Content-Length`、`Connection`、`Transfer-Encoding` 由系统管理，不能自定义覆盖。
- 请求头值在数据库中加密保存；管理员编辑服务商时会回显完整值，请仅向可信管理员开放后台权限。
...
>
> ⚠️ 更新版本时，应先同步全部源码再重新执行 `npm run build`，并完整替换 standalone 产物及 `.next/static`。不要只上传部分前端或 API 文件，否则可能出现页面与服务端接口版本不一致。数据库文件、`.env` 和 `public/uploads` 等持久数据应继续保留。
